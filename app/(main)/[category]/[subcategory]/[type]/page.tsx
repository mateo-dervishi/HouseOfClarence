...
export default function TypePage({ params }: TypePageProps) {
  ...
  // Find category, subcategory, and type from navigation data
  const category = navigationData.find((cat) => cat.slug === params.category);
  const subcategory = category?.subcategories.find(
    (sub) => sub.slug === params.subcategory
  );
  const productType = subcategory?.types.find(
    (type) => type.slug === params.type
  );

  // Filter products by category, subcategory, and type
  // We match type name in the product name or tags
  const displayProducts = useMemo(() => {
    if (!productType) return [];
    
    // First get products from this subcategory
    ...
    // Then try to filter by type - match type name in product name, tags, or other fields
    const typeName = productType.name.toLowerCase();
    const typeSlug = params.type.replace(/-/g, " ").toLowerCase();
    
    const typeProducts = subcategoryProducts.filter((product) => {
      ...
      return (
        ...
        productName.includes(typeSlug) ||
        productTags.includes(typeName) ||
        productTags.includes(typeSlug) ||
        material.includes(typeName.split(" ")[0]) || // Match first word (e.g., "ceramic" from "ceramic sinks")
        material.includes(typeSlug.split(" ")[0])
      );
    });
    
    // If we found type-specific products, return them. Otherwise return all subcategory products.
    return typeProducts.length > 0 ? typeProducts : subcategoryProducts;
  }, [params.category, params.subcategory, params.type, productType]);
  ...
  return (
    <main>
      ...
      <section className="relative h-[50vh] min-h-[380px] bg-[#f5f5f5] overflow-hidden">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <motion.h1 
            ...
            className="text-white text-3xl md:text-5xl tracking-[0.2em] font-light uppercase text-center px-4"
          >
            {productType.name}
          </motion.h1>
        </div>
      ...
      <nav className="max-w-[1600px] mx-auto px-6 py-6">
        <ol className="flex items-center gap-2 text-[12px] tracking-[0.05em] text-warm-grey flex-wrap">
          <li>
          ...
          <li>/</li>
          <li className="text-primary-black capitalize">{productType.name}</li>
        </ol>
      </nav>

      {/* Sibling Types Navigation */}
            ...
            {sortedProducts.length > 0 ? (
              ...
              <div className="text-center py-24">
                ...
                <p className="text-warm-grey text-sm mb-6">
                  Check back soon or browse our full {subcategory.name} collection.
                </p>
                <Link
                  href={`/${params.category}/${params.subcategory}`}
                  className="inline-block text-[13px] tracking-[0.1em] uppercase px-6 py-3 bg-primary-black text-white hover:bg-charcoal transition-colors"
                >
